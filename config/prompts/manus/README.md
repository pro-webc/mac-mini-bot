...
3. **TEXT_LLM（Claude CLI）生成ソース**（同上）。
...
`modules.basic_lp_refactor_claude.build_basic_lp_refactor_user_prompt` は次を**この順**で連結する（手作業と同じ本文＋プレースホルダ展開のみ。GitHub 垢の注意などは `orchestration_prompt.txt` に書く）。
