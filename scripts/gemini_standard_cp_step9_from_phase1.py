#!/usr/bin/env python3
"""
フェーズ1スナップショット（ヒアリング本文）を入力に、STANDARD-CP の Gemini **9/15（手順4・タブ⑤の1通目）**だけ実行する。

タブ④の成果物は不要。参考サイトURLブロックはヒアリング本文から ``reference_site_url_from_hearing`` で組み立てる（本番と同じ）。

::

  python3 scripts/gemini_standard_cp_step9_from_phase1.py \\
...
from modules.standard_cp_gemini_manual import run_standard_cp_gemini_api_call_9_of_15  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="STANDARD-CP Gemini 9/15（手順4・タブ⑤1通目）をフェーズ1入力で実行"
    )
    parser.add_argument(
        "--phase1-dir",
        type=Path,
    ...
    prompt, response = run_standard_cp_gemini_api_call_9_of_15(
    ...
    (out / "README.txt").write_text(
        "\n".join(
            [
                "STANDARD-CP Gemini 段階テスト（手順4・API 9/15・タブ⑤1通目）",
                "",
