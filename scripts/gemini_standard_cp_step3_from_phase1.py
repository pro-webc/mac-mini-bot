#!/usr/bin/env python3
"""
**2回目の応答**（手順1-3 相当の長文）を入力に、STANDARD-CP の Gemini **3/15（手順2・タブ③）**だけ実行する。

2回目の成果物フォルダ（``02_response_step_1_2_and_1_3.txt`` があるディレクトリ）を ``--prev-gemini-dir`` で渡す::

  python3 scripts/gemini_standard_cp_step3_from_phase1.py \\
    --phase1-dir output/pipeline_test_runs/<run>/phase1_snapshots/<UTC> \\
    --prev-gemini-dir output/pipeline_test_runs/<run>/gemini_step_tests/<2回目UTC>
...
from modules.standard_cp_gemini_manual import run_standard_cp_gemini_api_call_3_of_15  # noqa: E402
...
def _load_step1_3_response(prev_dir: Path | None, response_file: Path | None) -> str:
    ...
    if prev_dir is None:
        print(
            "ERROR: --prev-gemini-dir か --step1-3-response のどちらかが必要です。",
            file=sys.stderr,
        )
        sys.exit(1)
    d = prev_dir.resolve()
    ...
    if not inner.is_file():
        print(
            f"ERROR: {inner} がありません（--prev-gemini-dir は2回目の gemini_step_tests/<UTC>/ を指定）",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="STANDARD-CP Gemini 3/15（手順2・タブ③）を手順1-3応答で実行"
    )
    parser.add_argument(
        "--phase1-dir",
        type=Path,
    ...
    step13 = _load_step1_3_response(args.prev_gemini_dir, args.step1_3_response)

    prompt, response = run_standard_cp_gemini_api_call_3_of_15(step_1_3_output=step13)
    ...
    (out / "README.txt").write_text(
        "\n".join(
            [
                "STANDARD-CP Gemini 段階テスト（手順2・API 3/15・タブ③）",
                "",
