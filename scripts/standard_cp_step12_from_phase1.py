...
STANDARD-CP の Claude **12/15（手順7-1・タブ⑥・新規チャット）**だけ実行する。

::

  python3 scripts/standard_cp_step12_from_phase1.py \\
    --phase1-dir output/pipeline_test_runs/<run>/phase1_snapshots/<UTC> \\
    --prev-step-dir output/pipeline_test_runs/<run>/claude_step_tests/<11回目UTC>
...
from modules.standard_cp_claude_manual import run_standard_cp_claude_api_call_12_of_15  # noqa: E402
...
def _load_step6_response(prev_dir: Path) -> str:
    ...
    if not p.is_file():
        print(
            f"ERROR: {p} がありません（--prev-step-dir は11回目の claude_step_tests/<UTC>/ を指定）",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="STANDARD-CP Claude 12/15（手順7-1・タブ⑥・新規チャット）"
    )
    parser.add_argument(
        "--phase1-dir",
        type=Path,
    ...
    prev11 = args.prev_step_dir.resolve()
    r6 = _load_step6_response(prev11)

    prompt, response = run_standard_cp_claude_api_call_12_of_15(step_6_output=r6)
    ...
    (out / "README.txt").write_text(
        "\n".join(
            [
                "STANDARD-CP Claude 段階テスト（手順7-1・API 12/15・タブ⑥1通目）",
                "",
