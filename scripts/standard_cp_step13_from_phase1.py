...
STANDARD-CP の Claude **13/15（手順7-2・タブ⑥の2通目）**だけ実行する。

::

  python3 scripts/standard_cp_step13_from_phase1.py \\
    --phase1-dir output/pipeline_test_runs/<run>/phase1_snapshots/<UTC> \\
    --prev-step-dir output/pipeline_test_runs/<run>/claude_step_tests/<12回目UTC> \\
    --step3-1-dir output/pipeline_test_runs/<run>/claude_step_tests/<4回目UTC> \\
...
from modules.standard_cp_claude_manual import run_standard_cp_claude_api_call_13_of_15  # noqa: E402
...
def _load_step7_1_turn(prev_dir: Path) -> tuple[str, str]:
    ...
    if not p_p.is_file():
        print(
            f"ERROR: {p_p} がありません（--prev-step-dir は12回目の claude_step_tests/<UTC>/ を指定）",
            file=sys.stderr,
        )
        sys.exit(1)
    if not p_r.is_file():
        print(
            f"ERROR: {p_r} がありません（--prev-step-dir は12回目の claude_step_tests/<UTC>/ を指定）",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _load_step3_1_from_dir(d: Path) -> str:
    ...
    if not p.is_file():
        print(
            f"ERROR: {p} がありません（--step3-1-dir は4回目の claude_step_tests/<UTC>/ を指定）",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="STANDARD-CP Claude 13/15（手順7-2・タブ⑥2通目・チャット継続）"
    )
    parser.add_argument(
        "--phase1-dir",
        type=Path,
    ...
    prev12 = args.prev_step_dir.resolve()
    ...
    prompt, response = run_standard_cp_claude_api_call_13_of_15(
        step_7_1_prompt=p71,
    ...
    (out / "README.txt").write_text(
        "\n".join(
            [
                "STANDARD-CP Claude 段階テスト（手順7-2・API 13/15・タブ⑥2通目）",
                "",
