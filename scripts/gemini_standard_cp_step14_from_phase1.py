...
**5〜8回目**（手順3-2〜3-5）の応答を束ねて ``STEP_3_SUBPAGES_OUTPUT`` とし、
STANDARD-CP の Gemini **14/15（手順7-3・タブ⑥の3通目）**だけ実行する。

12回目・13回目の解決:
  ``--prev-gemini-dir`` は **13回目**フォルダ（``01_prompt_step_7_2.txt`` 等）。
  ``00_source.json`` の ``prev_gemini_dir`` から **12回目**を辿り手順7-1を復元する。
...
::
  ...
  python3 scripts/gemini_standard_cp_step14_from_phase1.py \\
    --phase1-dir output/pipeline_test_runs/<run>/phase1_snapshots/<UTC> \\
    --prev-gemini-dir output/pipeline_test_runs/<run>/gemini_step_tests/<13回目UTC> \\
    --step32-dir .../gemini_step_tests/<5回目UTC> \\
    --step33-dir .../gemini_step_tests/<6回目UTC> \\
    --step34-dir .../gemini_step_tests/<7回目UTC> \\
    --step35-dir .../gemini_step_tests/<8回目UTC>
...
from modules.standard_cp_gemini_manual import run_standard_cp_gemini_api_call_14_of_15  # noqa: E402
...
def _load_step7_1_from_prev12(prev12: Path) -> tuple[str, str]:
    ...
    if not p_p.is_file() or not p_r.is_file():
        print(
            f"ERROR: {d} に手順7-1の保存がありません（12回目の gemini_step_tests/<UTC>/ を指定）",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _load_step7_2_turn(prev13: Path) -> tuple[str, str]:
    ...
    if not p_p.is_file():
        print(
            f"ERROR: {p_p} がありません（--prev-gemini-dir は13回目の gemini_step_tests/<UTC>/ を指定）",
            file=sys.stderr,
        )
        sys.exit(1)
    if not p_r.is_file():
        print(
            f"ERROR: {p_r} がありません（--prev-gemini-dir は13回目の gemini_step_tests/<UTC>/ を指定）",
            file=sys.stderr,
        )
        sys.exit(1)
    return p_p.read_text(encoding="utf-8"), p_r.read_text(encoding="utf-8")
...
def _resolve_step7_1_from_chain(prev13: Path) -> tuple[str, str]:
    ...
    prev12 = data.get("prev_gemini_dir")
    if not prev12:
...
def _build_subpages_from_dirs(
    ...
    s4 = _read_response(d34, "02_response_step_3_4.txt", "--step34-dir")
    s5 = _read_response(d35, "02_response_step_3_5.txt", "--step35-dir")
    return (
        "\n\n=== 手順3-2 サービスページ ===\n\n"
        + s2
        + "\n\n=== 手順3-3 会社概要 ===\n\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="STANDARD-CP Gemini 14/15（手順7-3・タブ⑥3通目・チャット継続）"
    )
    parser.add_argument(
        "--phase1-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="DIR",
        help="12回目フォルダを明示（省略時は 13回目/00_source.json の prev_gemini_dir）",
    )
    parser.add_argument(
        "--step32-dir",
        type=Path,
    ...
    parser.add_argument(
        "--subpages-output-file",
        type=Path,
        default=None,
        metavar="PATH",
        help="STEP_3_SUBPAGES 相当の全文テキスト（4ディレクトリ指定と排他）",
    )
    parser.add_argument(
        "--run-dir",
        type=Path,
    ...
    prev13 = args.prev_gemini_dir.resolve()
    ...
    has_dirs = all(
        x is not None
        for x in (args.step32_dir, args.step33_dir, args.step34_dir, args.step35_dir)
    )
    has_partial_dirs = any(
        x is not None
        for x in (args.step32_dir, args.step33_dir, args.step34_dir, args.step35_dir)
    ) and not has_dirs
    has_file = args.subpages_output_file is not None

    if has_partial_dirs:
        print(
            "ERROR: --step32-dir / --step33-dir / --step34-dir / --step35-dir は"
            " 4つすべて指定するか、すべて省略して --subpages-output-file を使ってください。",
            file=sys.stderr,
        )
        sys.exit(1)
    if has_dirs and has_file:
        print(
            "ERROR: 4ディレクトリ指定と --subpages-output-file は同時に使えません。",
            file=sys.stderr,
        )
        sys.exit(1)
    if not has_dirs and not has_file:
        print(
            "ERROR: (--step32-dir … --step35-dir の4つ) または "
            "--subpages-output-file が必要です。",
            file=sys.stderr,
        )
        sys.exit(1)

    if has_file:
        p = args.subpages_output_file.resolve()
        if not p.is_file():
            print(f"ERROR: --subpages-output-file が見つかりません: {p}", file=sys.stderr)
            sys.exit(1)
        subpages = p.read_text(encoding="utf-8")
        sub_ref = str(p)
        via_file = True
    else:
        subpages = _build_subpages_from_dirs(
            args.step32_dir,
            args.step33_dir,
            args.step34_dir,
            args.step35_dir,
        ...
        sub_ref = {
            ...
            "step34": str(args.step34_dir.resolve()),
            "step35": str(args.step35_dir.resolve()),
        }
        via_file = False

    prompt, response = run_standard_cp_gemini_api_call_14_of_15(
        step_7_1_prompt=p71,
        ...
        json.dumps(
            {
                ...
                "prev_gemini_dir": str(prev13),
                "step7_1_dir": step12_ref,
                "subpages_source": sub_ref,
                "subpages_via_file": via_file,
            },
            ensure_ascii=False,
            indent=2,
        )
    ...
    (out / "README.txt").write_text(
        "\n".join(
            [
                "STANDARD-CP Gemini 段階テスト（手順7-3・API 14/15・タブ⑥3通目）",
                "",
                "00_source.json — phase1・run・13回目・12回目・下層ページ束ね参照",
