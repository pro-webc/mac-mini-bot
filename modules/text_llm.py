...
def text_llm_complete(
    *,
    ...
    sys_prompt = system or "あなたは指示に従うアシスタントです。"
    ...
    if provider == "claude_code_cli":
        ...
        return _complete_via_cli(cmd, sys_prompt, user)
...
def _complete_via_cli(command: str, system: str, user: str) -> str:
    ...
    try:
        r = subprocess.run(
            ...
            text=True,
            capture_output=True,
            timeout=int(os.getenv("TEXT_LLM_CLI_TIMEOUT_SEC", "1200")),
            env={**os.environ},
            check=False,
        )
    ...
    if r.returncode != 0:
        ...
        raise RuntimeError(f"TEXT_LLM CLI が失敗しました (exit {r.returncode}): {err}")
