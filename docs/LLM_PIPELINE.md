...
`main.py` の `WebsiteBot.process_case` を基準に、**LLM を実行する箇所**（Claude Code CLI・Manus 等）と **LLM を呼ばない前処理・組み立て** を分けて示す。

---

## 多段チェーンの設計思想

### なぜ 1 回の LLM 呼び出しでサイトを作らないのか

LLM に「このヒアリングからサイトを作って」と 1 回で頼むと、以下の問題が起きる:

- **情報設計が飛ぶ** — ページ構成やセクション設計をせず、いきなりコードに入る
- **コンテンツが薄い** — テキストライティングの工程がないため、見出しだけのセクションや Lorem ipsum が頻出する
- **デザインが場当たり** — 配色・トーン・余白が統一されず、セクションごとにバラバラになる
- **途中の品質制御ができない** — 1 回の呼び出しでは「情報設計だけやり直す」ことができない

### ステップ分解が品質を生む仕組み

本システムは**人間の制作マニュアルの手順**をそのまま LLM チェーンに変換している。STANDARD プラン（6 ページ）の 15 回チェーンを例にすると:

| 段階 | タブ | 手順 | やること | 入力 |
|------|------|------|----------|------|
| 情報構造化 | ①② | 1-1, 1-2+1-3 | ヒアリング整形・事業理解の凝縮 | 生のヒアリング |
| 情報設計 | ③ | 2 | 6 ページ構成・セクション設計 | 構造化された事業情報 |
| コンテンツ | ④ | 3-1〜3-5 | ページ別テキストライティング ×5 | ページ構成 + 事業情報 |
| デザイン | ⑤ | 4→5→6 | 配色 → トークン → 指示書 | テキスト + 事業情報 |
| 実装 | ⑥ | 7-1→7-4 | コード生成 ×4 | デザイン指示書 |

各ステップは**前のステップの出力を入力として受け取る**（`{{STEP_1_3_OUTPUT}}` 等のプレースホルダで注入）。これにより:

- **段階ごとの品質を独立に評価**できる（`output/<record>/llm_steps/003_*` = ページ構成の品質）
- **問題のあるステップだけプロンプトを改善**できる
- **チャットセッション管理**で、同一タブ内のページ間トーン統一と、新規タブでのコンテキスト集中を制御

### TEXT_LLM（Claude Code CLI）→ Manus の二段構成
...
| 2 | TEXT_LLM（仕様・台本） | **Claude Code CLI（マニュアル手順）** | `modules.llm.text_llm_stage` → 下表 | 各 `*_USE_CLAUDE_MANUAL` が true かつ `claude` CLI が利用可能なこと |
...
|----------|------|------|------------------------------|
| BASIC LP | `ContractWorkBranch.BASIC_LP` | `run_text_llm_stage` → `run_basic_lp_claude_manual_pipeline` | `BASIC_LP_USE_CLAUDE_MANUAL=true` かつ `claude` CLI が PATH にあること。モデルは `CLAUDE_BASIC_LP_MODEL` 等。未設定時は `RuntimeError` |
...
### TEXT_LLM（Claude Code CLI）マニュアルチェーンのあとに続く「最終リファクタ」（Manus）

各 `*_cp_claude_manual` / `basic_lp_claude_manual` の末尾付近で、設定が有効なとき **Manus API でタスクを 1 件**作成し、ポーリングして完了まで待つ。

- **モジュール**: `modules.basic_lp_refactor_claude.run_basic_lp_refactor_stage` → 内部で `modules.manus_refactor.run_manus_refactor_stage`
...
- **マニュアル本編**: TEXT_LLM は Claude Code CLI（`claude -p`、プラン別 `CLAUDE_BASIC_LP_MODEL` / …）
...
→ **テキストの多段生成は Claude Code CLI（サブスクリプション認証、API キー不要）。最終リファクタは Manus（`MANUS_API_KEY`）**。
...
`modules.basic_lp_refactor_claude.build_basic_lp_refactor_user_prompt` は次をこの順で連結する（プラン別の `preface_intro` は **使わない**）。本文は手作業用 `manus/*.txt` のみ（プレースホルダ展開）に揃える。
...
画像は **Manus（nanobananaPro）** が生成・配置する。
...
- `modules.llm.llm_pipeline_common` のサイト台本組み立て・パース（**生成モデルは呼ばない**。TEXT_LLM（Claude CLI）後の `spec` 整形などに使用）
...
|------|----------------------------------------|
| BASIC LP マニュアル本体（TEXT_LLM） | `CLAUDE_BASIC_LP_MODEL` |
| BASIC-CP マニュアル（TEXT_LLM） | `CLAUDE_BASIC_CP_MODEL`（未設定時は LP と同じ） |
| STANDARD-CP マニュアル（TEXT_LLM） | `CLAUDE_STANDARD_CP_MODEL` |
| ADVANCE-CP マニュアル（TEXT_LLM） | `CLAUDE_ADVANCE_CP_MODEL` |
| サイト用画像 | Manus（nanobananaPro）が生成。Bot 側の画像 API は未使用 |
