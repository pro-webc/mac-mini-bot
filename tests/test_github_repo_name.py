...
def test_sanitize_github_repo_name_record_test_partner() -> None:
    assert sanitize_github_repo_name("株式会社TS-hub", "16715") == "bot-16715-ts-hub"
    assert sanitize_github_repo_name("ignored", "42") == "bot-42-ignored"


def test_sanitize_github_repo_name_strips_non_alnum_record() -> None:
    assert sanitize_github_repo_name("", "R-99x") == "bot-R99x-partner"


def test_github_owner_repo_from_clone_url_https_git_suffix() -> None:
    o, r = github_owner_repo_from_clone_url(
        "https://github.com/propagate-webcreation/bot-123-ACME.git"
    )
    assert o == "propagate-webcreation"
    assert r == "bot-123-ACME"
